/* PDF export — a self-contained writer, behind the Executive Summary's
   "Export PDF" button.

   No library, no build step, no npm — the same constraint as the rest of the
   repo. It writes PDF 1.4 by hand using the two Core-14 fonts every reader
   already has, so nothing is embedded and nothing is fetched, and it works
   offline from the service worker's cache like any other page asset.

   The content is read off the LIVE DOM rather than re-described here: the
   Executive Summary is hand-authored prose that changes whenever the app does
   (CLAUDE.md's Documentation currency rule), and a second copy of it in a
   generator is exactly the drift that rule exists to prevent. Anything marked
   .no-pdf is skipped.

   Emoji and other characters outside WinAnsi are folded or dropped (see FOLD)
   — this app's copy is full of them, and a dropped glyph is preferable to a
   byte a reader rejects. Cooking Mode, timers and the tracker are untouched by
   any of this.

   MCPDF.fromElement(el, opts).build(label) -> a PDF string
   MCPDF.download(pdfString, filename)      -> saves it
   MCPDF.save(el, opts)                     -> both, the usual call. */
(function () {
  'use strict';
  ...
  window.MCPDF = { create: create, fromElement: fromElement, download: download, save: save, fold: fold };
})();
