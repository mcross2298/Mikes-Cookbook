...
| `cookbook-home.js` | Shell controller (~216 KB, ~4.8k lines). Renders Home hub, This Week meal planner (Plan my week, batch-prep suggestion, cook-log + macro history), Browse (collection **or** dish-type taxonomy + app-wide search + facet chips), Favorites, Mike's Favorites. Browse's search (and the planner's recipe-picker overlay) delegate ranking to `mc-search.js` (CI initiative 4) via `searchRecipes()` — a local `indexOf`-over-five-fields scan used to return **zero** results for `"chicken broccoli"` or `"chiken"`, verified against the real catalog before that file existed. Owns meal-plan + favorites logic. **Macro-target on-ramp (Pillar F follow-up, roadmap §2.4's own stated stretch):** the recipe-picker's `pickCard()` shows an opt-in "🎯 fit your goal" chip (`pickerMacroSuggestion()`, built on `mc-scale.js`'s `solveScaleForTarget()`) when the tracker has real daily goals set — a separate tap target from the card's existing default-serving add, so nothing changes for a cook who's never set tracker goals. Required changing `pickCard`'s root element from a `<button>` to a `<div role="button">` (a `<button>` can't legally nest the chip's own `<button>`), with a `keydown` handler preserving keyboard operability. **Pantry quantities (roadmap: "Real Pantry Inventory"):** `mc-cookbook:pantryqty` — `{ [pantryKey]: {qty, unit, ts} }` — is a new, separate, sparse store alongside the existing binary `mc-cookbook:pantry` Set, not a change to that store's own shape (the roadmap's original sketch described migrating `:pantry`'s array elements to objects directly; measuring the actual blast radius of that — every existing consumer of `loadPantry()`'s `Set<string>` return — showed a companion map gets the same "quantity is strictly additive, never required" guarantee with far less risk: a staple with no entry here behaves in every existing code path exactly as it always has). `loadPantryQty()`/`savePantryQty()`/`setPantryQty(item, qty, unit)` are the new storage helpers, all routed through `writeStore()` same as every other store; `togglePantry()` now also clears a staple's quantity entry when it's removed from the pantry Set entirely, so a later re-add never resurfaces a stale amount. **Wired into the This Week planner's grocery list** (first UI slice): `openPantryQtyEditor()` is a 📏-button overlay on any pantry-footer row for recording or clearing an amount; `pantryShortfall()` feeds `mc-grocery.js`'s per-row `need` (see that file's row below) and a recorded pantry amount into `MCPantry.compare()`, and a `"short"` result puts the staple back on the buy list with how much more is needed instead of the binary toggle's usual full suppression — `"unquantified"`/`"unknown"`, and any row `mc-grocery.js` itself can't reduce to one comparable `need`, fall straight through to that original behavior — **that fallback was silent until re-audit gap #03**: `pantryQtyDisabledReason(row)` now gives the 📏 control a one-line reason and a dimmed (not `aria-disabled`) state whenever `row.need` is null, so recording an amount that can't move the buy list is no longer indistinguishable from one that can. **Quantity-aware follow-up (Pillar F):** `pantryMatchInfo()` (the Recipes-screen "cook what you have" filter + low-shopping card badge) now calls `MCPantry.compare()` when a staple has a recorded quantity — genuinely `"short"` no longer silently counts as "have"; unquantified/uncomparable falls back to the original binary behavior. `pantryCandidates()` was investigated and left alone on purpose: it only ever considers items NOT already in the pantry Set, so quantity is out of scope for it by construction, not a gap. `recipe.html`'s Grocery tab's pantry-awareness lives in `mc-pantry.js`'s own row. **Recipe capture (roadmap Phase C):** Home's "Add Recipe" module now opens `openAddRecipeChooser()` — Type it in (straight to `MCRecipeForm.open()`, unchanged) or Paste a link (`openImportFromUrl()`). The latter is the orchestration glue for the pipeline built across #146/#147: checks `MCNet.isOffline()` and `MC_SB.currentUser()` first (with a one-tap "Open sign-in" affordance — `.home-account-btn` is guaranteed to exist in the DOM at this point since Add Recipe is itself a Home-only module) before calling `MC_SB.callFunction("fetch-recipe-source", {url})`, then `MCImport.parseFromHTML()` on the returned HTML, then `MCRecipeForm.open(parsed.recipe)` — never auto-saves, always lands on the same editable form a hand-typed recipe would. `importErrorMessage()` maps the edge function's error codes to cook-facing text rather than surfacing a raw reason string. Photo/OCR capture (the roadmap's third path) is deliberately not built. `readSharedRecipeParams()`/`handleSharedRecipe()` receive the Web Share Target params `manifest.json` registers (runtime-invisibles audit) — read and stripped from the URL as the very first statement in `init()`, before `setTab()`'s own `history.replaceState` can wipe the query string first (a real ordering bug caught while building this: reading them at the end of `init()`, after the default screen's `setTab()` call, meant they were already gone). **What did *not* move (audit C-08):** the planner. It is ~1,350 contiguous lines and looks like the obvious first extraction — the audit said so — but it references **66** names defined elsewhere in this file, so lifting it would mean threading a 66-entry context object through the seam. Contiguous is not separable. The two regions that did move needed five and four hooks respectively; that ratio is the test to apply before extracting anything else here. **One planning door (audit C-05):** `openPlanWeek()` is the single overlay behind the Plan pane's "✨ Plan my week" button. It replaced `openSmartWeek` + `openBandwidthQuiz` — two full-screen overlays reached from two sibling buttons, with Smart Week carrying its own Balanced/Macro toggle on top: four entry points for one grid, and the choice was irreversible in the UI (comparing two meant cancelling out and starting from a different button). The bias is now a chip row (**Balanced · Macro · Time**) that regenerates in place; Macro still only appears when real macro goals exist. Time keeps its day-budget quiz, shown inline until at least one day is assigned, with an "‹ Adjust days" way back; assignments still persist in `mc-cookbook:timecheck`. The two overlays also carried a near-identical ~60-line day-grid renderer — that's `pwDayBlocks` now. **Re-audit gap #04:** `wirePhotoSync()` listens for `mc-photos.js`'s `mc:photosready` event and re-renders whichever shell screen is currently active (Home, Browse, Favorites, Mike's Favorites, or the planner) — deliberately NOT routed through `onDetailReady`/`fireDetailReady` above, since those are scoped to `mc-data.js`'s shard arrival (a different async dependency, gated on `MCData.allReady()`) and firing them early on photo-readiness would repaint panes still genuinely waiting on shards for an unrelated reason. Broader than `wireFavSync()`'s two-screen list on purpose: a photo can appear on a card on every one of those screens, not just the two that care about favorite status specifically.
...
| `collection.html` / `collection.js` | One collection's recipe list (`collection.html?c=<id>`) + live search — ranked via `MCSearch.search()` (CI initiative 4), replacing a local indexOf-over-three-fields scan; coming-soon placeholder for future collections; also serves the "My Recipes" collection. **Re-audit gap #04:** the existing `detailRepaint` slot (set by `onDetailReady()` for shard-arrival repaints) is reused for `mc-photos.js`'s `mc:photosready` event too — same "just re-run `paint()`" shape as the shard-arrival case, no new mechanism needed here. |
...
| `mc-photos.js` | **The photo library — re-audit critical gap #04.** Every recipe cover photo and cook-log photo has always been a base64 JPEG string in `localStorage`, capped at `MAX_RECIPE_PHOTOS` (24) and `MAX_PHOTOS` (12) purely because that store is the wrong primitive for binary blobs — those ceilings were never a product decision, just localStorage's shared origin quota worked backward into a count. Worse: `mc-cookbook:cooked` (photo bytes and all) sits in `mc-sync.js`'s synced `STORES` whitelist, and that file's own comment claimed cook-log photos were "device-local by design" — untrue of the code as shipped, since `push()` sent whatever `JSON.parse(localStorage.getItem(key))` returned, unmodified; every cook-log photo's full base64 body was pushed to Supabase's `user_sync` table on every sync. Moves both stores' bytes into IndexedDB (database `mc-cookbook-photos`, one object store `photos`, keyed `cover:<recipeId>` / `cook:<recipeId>:<at>`, `bytes` stored as an `ArrayBuffer` rather than a `Blob` to sidestep a documented old-WebKit structured-clone bug) — no item cap, and mc-sync.js needed no change at all to become an honest description of its own code again, since the bytes are simply no longer in the JSON it reads. **`mc-cards.js`'s `photoFor()` stays fully synchronous** despite IndexedDB being async: this module warms an in-memory `Map<id, objectURL>` once at load, `urlFor()` reads it synchronously (`null` while warming or genuinely absent — exactly what "no photo" already meant to every caller), and a `ready` Promise + `mc:photosready` DOM event let each of the three page controllers re-run its own existing repaint entry point once warm (`cookbook.js`'s `renderHero()` + the new narrowly-scoped `refreshPhotoWidget()` for the eyebrow "add a photo" control; `cookbook-home.js`'s `wirePhotoSync()`, which re-renders whichever shell screen is active; `collection.js`'s existing `detailRepaint` slot) — no new repaint mechanism invented, same pattern `mc-data.js`'s shard loading already established. **Migration never lets a photo go dark mid-transition:** `runMigration()` is one-time (flag-gated: `mc-cookbook:photosMigratedV1`), moves whatever legacy base64 is still in `mc-cookbook:photos` / `mc-cookbook:cooked[id][].photo` into real IndexedDB records, then strips the base64 back out (a cook-log entry's `.photo` becomes the boolean `true` — a "photo exists" marker every existing truthy check keeps working on unchanged); `mc-cards.js`'s `photoFor()` checks for a still-present raw legacy string FIRST and uses it directly if found, falling through to this module's warm cache only once that string is actually gone. Also called **unconditionally** (not flag-gated) by `mc-export.js`'s restore path, since importing an old backup reintroduces raw base64 on a device that already migrated once — a second pass is a fast no-op. **The backup decision, made deliberately:** a JSON export now naturally captures an empty cover map and boolean-only cook-log markers (it always only ever wrote raw localStorage strings); re-inflating every IndexedDB blob back into the backup file was declined — it would undo the entire point of the move for exactly the file a cook is most likely to keep several copies of. A restored OLD backup still gets every photo it was made with; a NEW photo taken after that backup was made is never in it, same as it never was for any other reason either. Exposes its pure, Node-testable half (id construction, the legacy-value-detection floor) via `module.exports`, pinned by `tools/test-mc-photos.js`; the IndexedDB-dependent half can only be exercised by a real browser and is proven by `tools/smoke-test.js`. Exposed as `window.MCPhotos`. |
| `mc-setlog.js` | **Per-item add/remove states for the three set-shaped stores** (re-audit critical gap #01). `mc-cookbook:favorites`, `mc-cookbook:pantry` and `mc-cookbook:mealplan:grocery` are all `Array.from(Set)` id lists synced by `mc-sync.js`'s `stringSet` strategy — a pure **union**, which carries additions and cannot express a **removal**. Measured directly against the shipped merge function: un-checking a grocery row (local `["eggs"]`, remote `["milk","eggs"]`) merges back to `["eggs","milk"]`, so milk returns to the cart; same for un-favouriting a recipe and removing a pantry staple. The information was destroyed in the store itself before sync ever ran, so no retry or reconnect recovered it — and the Executive Summary meanwhile promised favourites and pantry "follow you across devices." **The fix is a per-item last-write-wins register kept as a SEPARATE, SPARSE COMPANION** (`mc-cookbook:setlog` → `{ "<kind>:<id>": {on:0\|1, ts} }`), not a change to those three stores' own shape — the same call `:pantryqty` made against `:pantry`, for the same reason: the array shape is read by `MCFav.load()`, `loadPantry()`, `loadGroc()`, `pantryMatchInfo()`, `pantryCandidates()`, the low-shopping filter, the grocery-row suppression, `mc-grocery.js`'s `configure()` hooks and `mc-export.js`, and every one keeps working untouched. An id with no entry here behaves exactly as it always has. **`on:1` is load-bearing, not bookkeeping:** a bare tombstone map cannot express remove-then-re-add, so the stale tombstone would strip the item again the moment it came back; recording both states makes each entry an ordinary LWW register, which `mc-sync.js`'s existing **`mapByTs`** strategy already resolves per key — **no new merge function**. Written by **diffing at each store's single `save()` choke point** rather than at every toggle: all three saves take a whole Set, so a caller-side hook would have to be threaded through every toggle, bulk clear and anything added later; diffing what's being written against what's on disk catches all of them, including callers that don't know this file exists. Only changed ids get an entry, so the log is a record of edits, not a mirror. Two deliberate limits, both in the file header: `prune()` drops entries past 90 days (a device offline longer than that still resurrects — the old behaviour, now *bounded* rather than permanent), and a same-millisecond edit to the same id on two devices resolves by `mergeMapByTs`'s `>=` tiebreak and can disagree (inherited from that strategy, not new). Pinned by `tools/test-mc-setlog.js`. Exposed as `window.MCSetLog`. |
...
| `mc-pantry.js` | **The pantry quantity comparison engine — first slice of `FLAGSHIP_COOKBOOK_ROADMAP.md`'s "Real Pantry Inventory + Dynamic Substitution."** Pure, no DOM/storage, built directly on `mc-units.js`'s `resolveUnit()` (both the recipe's need and the pantry's have amount are bucketed through the exact same function `mc-grocery.js` uses to merge a shopping list, so "are these two amounts comparable" is answered identically in both places). `MCPantry.compare(mergeName, needQty, needUnit, haveQty, haveUnit)` returns one of four states, not the usual three — `"unquantified"` (the pantry has this staple but no recorded amount — the binary case every existing pantry consumer already handles, and must keep behaving exactly as it does today, never treated as zero) sits alongside `"enough"` / `"short"` (carries how much more is needed, shaped differently for a real vol/wt conversion vs. a raw-unit or bare-count mismatch) / `"unknown"` (mc-units.js's own "leave it fragmented rather than guess" honesty, e.g. a genuine unit-family mismatch). **Wired as of the This Week planner's grocery list** (see `cookbook-home.js`'s row above), and now **`recipe.html`'s Grocery tab too** (tri-state substitution slice) — `cookbook.js`'s `renderGrocery()` calls `compare()` for any ingredient with a known `mc-search.js` substitution and a recorded pantry quantity, inside the existing "Don't have it on hand?" card only (the shopping list itself stays a pure list, unchanged — Phase 3 §3.1). `"enough"` drops the note (reduces noise), `"short"` names both amounts, `"unquantified"`/`"unknown"`/no entry fall back to the original generic note. `pantryMatchInfo()`, `pantryCandidates()` and `groceryItemCount()` (the Recipes-screen "cook what you have" filter, the "you keep buying this" nudge, and Home's grocery-count badge) stay binary-only — extending each to be quantity-aware is real, separate design work, not a mechanical follow-up. **`compare()`'s own math is unchanged by re-audit gap #03** — its `"unknown"` branch (need and have resolve to genuinely different unit families, e.g. a volume need against a weight-recorded pantry) was flagged in the re-audit as a candidate for "routing through the DENSITY table," but that table only ever bridges a COUNT WORD to a weight (`"3 cloves"` → grams) — `resolveUnit()` already applies it automatically before `compare()` ever runs, so there is no additional density-routing left to add without inventing volume↔weight data this app has never curated. Left exactly as honestly incomparable as before; what gap #03 actually fixed is the caller-side silence around `mc-grocery.js`'s `need: null` (see that file's row and `cookbook-home.js`'s `pantryQtyDisabledReason()`). Pinned by `tools/test-mc-pantry.js` against real `mc-units.js` conversions, a real density-bridged case (garlic cloves), a raw-unit case, a bare-count case, and two genuine-incomparability cases; `tools/test-mc-grocery.js` covers the integration end to end through `mc-grocery.js`'s `need` field (see that file's row). Exposed as `window.MCPantry`. |
| `mc-grocery.js` | **The merged grocery list's quantity math** (audit C-08): parsing an amount (`"1 1/2"`, `"0.75"`, `"to taste"`), summing across planned meals, purchase-unit conversion, and the ingredient-identity keying (`category|singularized-name`) that both the grocery merge **and** Smart Week's ingredient-overlap scoring read — one definition, two callers. **CI initiative 2** moved unit conversion, the density table and aisle assignment out to `mc-units.js`; `buildGrocery()` now bucket-keys each ingredient line through `MCUnits.resolveUnit()` and groups rows by `MCUnits.aisleFor()` instead of the raw category, returning `{ aisle, rows }` (was `{ category, rows }`) with each row optionally carrying `derived` — deduplicated, human-readable notes (`"1/2 each ≈ 29 g"`) for any sub-amount that came from a density estimate rather than an authored quantity. Takes six readers via `MCGrocery.configure()` (`recipes`, `recipeById`, `planMeals`, `loadPantry`, `pantryKey`, and — as of the Pillar F follow-up pass — `loadPantryQty`). **Pantry quantities (roadmap: "Real Pantry Inventory," first UI slice):** each row also carries `need: {qty, unit} | null` — a single (qty, unit) pair reusable, unmodified, as `MCPantry.compare()`'s `needQty`/`needUnit` — but only when exactly one bucket contributed a real number to that row; a row this file itself fragments across multiple unit families is left `need: null` rather than guessing which bucket is "the" need, and the caller (`cookbook-home.js`) falls back to the original binary pantry behavior for those. `prettyMeasure()` is also exported now, for formatting a `MCPantry.compare()` `"short"` result. **`groceryItemCount()` is quantity-aware as of the Pillar F follow-up:** a staple flagged but genuinely `"short"` per `MCPantry.compare()` (using the new `loadPantryQty` hook) still counts toward the badge instead of silently suppressing; unquantified/uncomparable rows fall back to the original binary behavior unchanged. **`need: null` is common, not an edge case (re-audit critical gap #03):** measured at 235/854 (27.5%) of merged rows across the real corpus — any staple a cook measures one way in one recipe and another way in a second (a cup in one dish, a pound in another) leaves its row `need: null` by this file's own "one comparable bucket or nothing" rule above, which is correct and untouched by the fix. What was wrong sat entirely in the caller: `cookbook-home.js`'s 📏 control looked identical whether or not a row's `need` existed, so a cook could record a real amount against a `need: null` row and `pantryShortfall()` would bail on `!row.need` before ever reading it — the recorded amount silently could not move the buy list. `pantryQtyDisabledReason(row)` (in `cookbook-home.js`, next to `pantryShortfall()`) is the fix: a one-line reason whenever `row.need` is null, surfaced as a dimmed, still-tappable 📏 (CSS `.disabled` only — deliberately not `aria-disabled` or the `disabled` attribute, both of which make a control unactionable for a screen reader **and** for Playwright's own actionability check, which is exactly what this file's `tools/smoke-test.js` scenario caught while writing it). Nothing about the underlying math changed — a row that was `need: null` still is; the fix is entirely that recording an amount against one no longer looks like it did something when it didn't. Pinned by `tools/test-mc-grocery.js` (the `need` field itself, unchanged) and `tools/smoke-test.js` (the disclosure, found and verified against two REAL corpus recipes searched for at test time, never hardcoded IDs). |
| `mc-scale.js` | **Bi-directional macro/ingredient scaling — `FLAGSHIP_COOKBOOK_ROADMAP.md` §2.4.** Pure, no DOM. Scaling has only ever flowed one direction (pick a serving count → ingredients scale); `solveScaleForTarget(recipe, servingBase, target)` adds the reverse: given a macro target (`{protein_g?, calories?, carbs_g?}`, protein-first priority when more than one is given, this app's own stated bias), it returns `{scale, achieved, exact, driver, wanted}` — `scale` is an equivalent serving count reusable, unmodified, as `cookbook.js`'s existing `ingredientsFor(r, scale)`, since that function already scales any count (fractional included) with no authored tier by falling back to the native tier — this file doesn't duplicate that math, only decides what count to feed it. **Honesty constraint:** `exact` is only true when EVERY requested field lands within 5% of its target at that one linear scale factor — a target combination whose ratios don't match the recipe's own macro ratios comes back `exact:false` with the real achieved numbers, never presented as a match. Wired into `recipe.html`'s existing serving stepper as a **Servings / Hit a macro target** toggle (`renderScaleModeToggle`/`renderScaleTargetPanel` in `cookbook.js`) — only shown when the recipe has real macro data; a macro-free recipe's servings stepper is completely unaffected. Checklist state stays keyed by the nominal serving count even in macro-target mode — only the amounts shown scale. **Second caller (Pillar F follow-up — the roadmap's own stated §2.4 stretch):** `cookbook-home.js`'s recipe-picker card (`pickerMacroSuggestion()` in `pickCard()`) calls the same `solveScaleForTarget()` to show an opt-in "🎯 fit your goal" chip when the tracker has real daily goals set, targeted at a flat one-third split across the "All" scope's three named slots — a separate tap target from the card's normal add, so a cook who never sets tracker goals sees zero change. Loaded on `index.html` too as of this second caller. Pinned by `tools/test-mc-scale.js`. Exposed as `window.MCScale`. |
| `mc-recipe-form.js` | **The "Add Recipe" overlay** (audit C-08) — title, icon, category, structured ingredients, numbered steps, and an optional per-serving **Nutrition** section, persisted via `MCUser`. Needs four hooks (`categoryOrder`, `closePicker`, `recipes`, `setTab`); `MCRecipeForm.open(prefill)` is the single entry point. `prefill` is optional and every field on it is independently optional: Web Share Target (`cookbook-home.js`'s `handleSharedRecipe()`) still only ever passes `{ title, description }`, but the URL-import flow (`openImportFromUrl()`, via `mc-import.js`'s parsed shape) can pass the fuller `{ title, description, prep_time_mins, cook_time_mins, base_serving, ingredients, steps, macros, via }` — ingredient/step rows are built from `prefill.ingredients`/`prefill.steps` when present instead of the usual two blank rows, `via` renders as a small "Imported from…" note under the title, and `macros` now prefills the editable Calories/Protein/Fat/Carbs fields (a real gap this closed: the form used to only *show* a parsed import's detected macros — "not saved yet; this form doesn't have a macros field" — with nowhere to actually save them; `user-recipes.js`'s `build()` now writes whatever's filled in, identical across `serving_2`/`serving_4` per the app's own per-serving-constant invariant). `category`, `icon` and `accent` are never prefilled from either source — a parser can't reliably guess them, and the cook picks in one tap regardless. **Re-audit critical gap #05** added a 5th, independently-optional Nutrition field — **Fiber (g)** — prefilled from `prefill.macros.fiber_g` same as the other four; its own row (`.rf-macro-row`, a variant of `.rf-times` that wraps instead of squeezing 5 flex items under a comfortable width) rather than reusing `.rf-times` outright, since that class is shared with the unrelated 3-field Prep/Cook/Servings row below it. |
| `mc-import.js` | **Recipe capture parsing — `FLAGSHIP_COOKBOOK_ROADMAP.md`'s "Recipe Capture & Import Pipeline," now wired end to end for the URL path.** Pure, DOM-free (regex over raw HTML text, not a `DOMParser`, so it behaves identically in a browser and under Node in CI): given fetched page HTML, returns a best-effort recipe shaped like `mc-recipe-form.js`'s own save payload, including a `macros` field that form now actually saves (see that row above — it used to only show it). Two paths — schema.org/Recipe JSON-LD (reliable, structured: `recipeIngredient`, `recipeInstructions` including nested `HowToSection`, `nutrition`, ISO 8601 `prepTime`/`cookTime`) tried first, then a heuristic "Ingredients"/"Instructions" heading-and-`<li>` scan that returns `null` rather than a half-built guess when it can't find both sections. Ingredient-line splitting (`parseIngredientLine`) and per-ingredient `category` (`guessCategory`, keyword-based) are both explicitly best-effort. **Re-audit critical gap #05:** the `nutrition` block's `fiberContent` is now parsed into `macros.fiber_g` alongside the other four fields — it was being read off the page and dropped on the floor before `mc-recipe-form.js`'s Nutrition section had a field to put it in, even though plenty of real recipe pages' schema.org markup carries it for free. Called by `cookbook-home.js`'s `openImportFromUrl()` after `fetch-recipe-source` returns HTML; **no OCR/photo-capture path exists** — that third capture method from the roadmap is deliberately unbuilt, same "ship the narrowest version" discipline as everything else in this pipeline so far. Pinned by `tools/test-mc-import.js` against JSON-LD and heuristic fixtures, a malformed-JSON-LD-must-not-crash case, a neither-section-found-must-return-null case, and (gap #05) both a `fiberContent`-present and a `fiberContent`-absent case — the latter proving `fiber_g` comes back honestly `null`, not `0` or silently missing from the object, when the source page simply doesn't publish it. Loaded in `index.html` right before `mc-recipe-form.js`. Exposed as `window.MCImport`. |
| `supabase/functions/fetch-recipe-source/index.ts` | **The server-side half of the capture pipeline's URL path — live and now called by the app.** A narrow Deno edge function on the shared Supabase project (`dhlxmoyjfxohbeiexwnr` — the same one `mc-supabase.js` points at, see the `mc-supabase.js` row below): given `{url}`, fetches it server-side and returns `{ok, html, finalUrl}`, since a static site can't fetch an arbitrary third-party URL client-side past CORS. Does no parsing — that stays `mc-import.js`'s job. **Hardened against SSRF**, since it fetches whatever URL a caller sends: every hostname (the initial URL *and* each redirect hop, followed manually via `redirect: "manual"`, never auto-followed) is DNS-resolved and checked against private/loopback/link-local/reserved ranges — including `169.254.169.254`, the common cloud-metadata address — before being fetched; only http/https, only default ports, no embedded credentials; an 8s timeout and a 3MB response cap (truncates rather than hangs). Deployed with **`verify_jwt: true`** (signed-in cooks only) — the project already has public utility functions (`food`, `parse-food`), but the deploy tool's own guidance defaults a *new* function to JWT-required unless it implements its own auth, and a URL-fetch proxy is a meaningfully larger abuse surface than a food lookup; this is a deliberate, flagged trade-off (recipe import via URL needs sign-in even though hand-typing one doesn't) the "Paste a link" flow now surfaces honestly (`cookbook-home.js`'s `openImportFromUrl()` checks `MC_SB.currentUser()` first and offers a one-tap path to sign in rather than failing opaquely). CORS is allow-listed to `https://mcross2298.github.io` plus localhost for local dev — worth being honest that CORS is a browser-enforced boundary, not a defense against a non-browser caller; the SSRF/size/timeout hardening plus the JWT gate are what actually bound abuse. Called via `MC_SB.callFunction()` (`mc-supabase.js`'s `client.functions.invoke()` wrapper, which attaches the signed-in user's session token automatically). Not precached by `tools/build-sw.py` (server code, not a client asset — that script's top-level scan is deliberately non-recursive and `.ts` isn't a precached extension anyway). **Covered by CI as of the end-to-end audit's A-04 follow-up** — `tools/test-fetch-recipe-source.js` is the repo's first gate on any server-side code. It still runs under Node, not Deno (nothing here does), so it reads this file's own source text and exercises the real `isPrivateIPv4`/`isPrivateIPv6`/`validateUrl`/`isAllowedOrigin`/`hostIsSafe`/`safeFetchHtml` bodies against a fake DNS resolver and a fake `fetch` — see that file's row for why it reads the text rather than importing or exporting anything. **Do not add an `export` here to make testing easier:** the committed file would then differ from the deployed function until someone redeploys, which is not a trade worth making on a security boundary. What's still uncovered is unchanged and narrower than before: live HTTP behavior against the *deployed* function can't be exercised from a session whose egress blocks `*.supabase.co`. |
| `recipe.html` / `cookbook.js` | Unified recipe detail: fixed header (title/tags/times/serving stepper) + swipeable sub-tabs (Overview & Macros · Grocery · Recipe). Owns serving scaling (`scaleQuantity`, wired for arbitrary 1–12 servings), check-off state, screen wake lock, and full-screen Cooking Mode. **`prettyNumber()` never renders a real amount as `"0"` (end-to-end audit)** — it used to, for any scaled quantity under the 0.06 fraction floor with no whole part, which is exactly what scaling a batch-yield recipe *down* produces; `smallAmount()` is the floor, and it stays numerically parseable because `mc-grocery.js` re-parses these strings. **`scaleQuantity()` scales a RANGE too (re-audit gap #02)** — it used to return any string `parseQtyNumber()` couldn't read verbatim, which is right for `"to taste"` and `"pinch"` and a silent arithmetic failure for `"2-3 cloves"`: cook a 2-serving recipe for eight and every line quadrupled except that one, unflagged, and read aloud verbatim by Cooking Mode's `speakIngredients()`. Both endpoints now scale, and the result joins with **`" to "`, not a hyphen** — a scaled range often lands on a mixed number, and `"1 1/2-3 cups"` is ambiguous on screen and spoken as nonsense, where `"1 1/2 to 3 cups"` reads correctly both ways. Both sides must parse as real numbers, which is exactly what keeps `"8-inch"`/`"4-inch"` — pan dimensions mis-keyed into `quantity` — out of the range path. **`prettyNumber`, `smallAmount`, `parseQtyNumber` and `scaleQuantity` are duplicated verbatim in `user-recipes.js`** (which generates a cook's own 2-/4-serving tiers) — fix both or neither; `tools/test-scaling-format.js` now compares the two **source texts** for those last two, not just `prettyNumber`'s behaviour, because a one-sided range fix passed the old behavioural check. (`user-recipes.js` used to inline the three parse patterns inside its own `scaleQuantity`; it carries the same `parseQtyNumber` now so both copies can be byte-identical.) Not extracted to a shared module because `user-recipes.js` loads *before* `mc-units.js` on all three page types and `collection.html` doesn't load `mc-units.js` at all, so a 17-line formatter would cost a script-order change on every page (cf. `mc-cards.js`, extracted at ~300 lines). If the detail shard fails to load (dead network on a cold cache), the Grocery/Recipe panes say so honestly and `mc-net.js`'s `mc:datareloaded` event — fired the instant the browser reports `online` — retries and re-renders automatically, no manual reload (runtime-invisibles audit, finding C3); `renderDetailReady(r)` is the shared render path both the first successful load and that retry call. **Timers are no longer owned here** — `timerChip()` is a thin view over `mc-timers.js`, and the Web Audio ping / vibrate / expiry handling moved there with them so one definition serves every surface. `?cook=1` opens Cooking Mode directly (the page used to parse only `?id` and always `setTab("overview")`, so hands-free mode sat four taps deep behind the third sub-tab with no way to link to it); Home's Today card links there. Cooking Mode's voice grammar also covers timers now — *"set a timer for N minutes"*, *"how long left"*, *"stop the timer"* — since setting one was the last thing that still required a tap. Groceries render **by aisle** via `MCUnits.aisleFor()` (CI initiative 2; this comment used to *say* "grouped by aisle" while actually grouping by the raw category enum — now true). The Grocery tab also renders a **"Don't have it on hand?"** card (CI initiative 4, `MCSearch.substitutionFor()`) — one informational line per ingredient this recipe uses that has a known swap in the curated `SUBSTITUTIONS` table, now **pantry-aware** (roadmap §2.2 tri-state slice — see `mc-pantry.js`'s row) rather than shown unconditionally; the underlying list itself is still not interactive. The header's serving stepper gains a second mode when the recipe has real macro data — **Servings** (unchanged) vs. **Hit a macro target** (roadmap §2.4, `mc-scale.js`'s `solveScaleForTarget()`) — up to three optional protein/calorie/carb fields that rescale the ingredient list to match, with the achieved macros and an honest "closest achievable by scaling" note when not every target can be hit at once; check-off state stays keyed by the nominal serving count regardless of which mode is active. **Re-audit critical gap #05** added a **Fiber** cell and a **Net Carbs** cell (`carbs_g - fiber_g`, clamped at 0) to the Macro Profile card — `renderMacros()` shows them only when `macro_profiles.fiber_g` is actually present on that recipe; invisible for all 318 built-ins today, since this app has no curated fiber data for them, same "leave it honestly absent rather than guess" discipline `mc-units.js`'s DENSITY table already follows. **CI initiative 3** added a **hero photo** — `renderHero()`, in its own `#hero` block placed *before* the sticky `#header` so it scrolls away naturally instead of permanently eating screen space, using the same `MCCards.photoFor()` chain a card does. Renders nothing for the 318 recipes with no photo. Controls depend on provenance: an authored photo is display-only; a cook-log-sourced photo gets a "☆ Set as cover" promote action; only an explicit cover gets the full replace/remove pair the small eyebrow widget always had (that widget now hides itself once the hero owns display — it's `renderPhotoWidget()`'s only remaining job is the "add a photo" entry point for a recipe with none). **Re-audit gap #04** moved both this hero's photo and every cook-log photo into `mc-photos.js`'s IndexedDB-backed library — `attachRecipePhoto()`/`removeRecipePhoto()`/`attachPhoto()` all now call `MCPhotos.setCover()`/`removeCover()`/`addCookPhoto()`, and `MAX_RECIPE_PHOTOS`/`MAX_PHOTOS` (the old localStorage-quota-driven 24/12-item caps) are retired outright rather than raised — the bytes no longer share a budget that needed a cap. Because `photoFor()`'s IndexedDB warm-up is async, `renderPhotoWidget()` can be called (on `renderHeader()`'s first synchronous pass) before the cache is warm and wrongly decide "no photo yet, show the add-photo button" for a recipe that already has one; `refreshPhotoWidget(r)` is a narrowly-scoped fix — same reasoning as `renderHero()` staying its own small repaint rather than a full `renderHeader()` rebuild — that swaps only the eyebrow widget in place once the `mc:photosready` event fires, found and fixed via `tools/smoke-test.js`'s own extended photo scenario rather than assumed safe. Also added **Counter Mode** — a manual, persisted (`mc-cookbook:countermode`) override independent of the ambient light/dark theme, toggled from a ☀︎ button in Cooking Mode's top bar, that redefines `.cook`'s own `--bg`/`--on-dark`/`--line-dark` to near-brutalist white-on-black with a 1.5rem type floor; `--accent` is deliberately untouched so it still means something (the progress bar, an active timer) instead of competing with max contrast for attention. |
...
| `user-recipes.js` | "My Recipes" — lets a cook add their own recipes from the Home hub; stored in `localStorage` (`mc-cookbook:userrecipes`) and merged into `window.RECIPES`/`COLLECTIONS` at load so they behave like built-in recipes everywhere (search, planner, favorites, categories). `build()`'s `macro_profiles` was hardcoded empty ("macro-free by design") until `mc-recipe-form.js`'s optional Nutrition section gave it something to save — an optional `raw.macros` (`{calories?, protein_g?, fat_g?, carbs_g?, fiber_g?}`, per single serving) now populates the SAME object onto both `serving_2`/`serving_4`, unscaled, matching `recipes-data.js`'s own per-serving-constant invariant; still empty by default for a recipe with nothing entered. **`fiber_g` (re-audit critical gap #05) is independently optional** — `macroProfile()`'s per-field loop writes whichever of the five the cook actually filled in, so a recipe can carry fiber alone, the other four alone, all five, or none. Must load after `recipes-data.js`, before the page controllers. |
| `tracker.js` / `tracker-store.js` / `tracker-calc.js` / `tracker-foodapi.js` / `tracker-barcode.js` / `tracker-recipe.js` | The in-app macro tracker (`#screen-tracker`): week calendar strip, hour-by-hour food log, calorie/macro goals from a suggest-then-adjust calculator, food entry via Open Food Facts search or barcode scan, and direct recipe logging from the recipe page. Store is `mc_macros_v1` — the same key and shape 4-Weeks-to-Open-'s workout app uses, so a signed-in trainee's tracker data is one store, not two (see Client-side state below). **Re-audit critical gap #05, corrected from the original audit's framing:** investigating found no day-level fiber or net-carbs total anywhere in the shipped app — fiber only ever appeared in `tracker.js`'s single-item Nutrition Facts sheet (`openFacts()`), sourced only from Open Food Facts' `nutr.fiber`, and even there it already rendered an honest "—" (not "0") for a recipe-sourced entry, since that entry's `nutr` was always the literal empty object `{}`. The real, closeable gap was structural: `recipes-data.js` had no field to carry fiber at all, so it could never reach a recipe-logged entry even when genuinely known. `tracker.js`'s `foodFromRecipe()` now looks up `macro_profiles`'s `fiber_g` (when a recipe authors it) and carries it into `nutr.fiber`, exactly the shape every other food source already uses — no second shape invented. `openFacts()` gained a **Net Carbs** row (`carbs - fiber`, clamped at 0) next to Fiber, shown only when that specific item's fiber is actually known. `tracker-recipe.js`'s `perServing()`/`openSheet()` (the recipe page's own "Log to tracker" FAB — a third, separate near-duplicate of the same tier-extraction logic `tracker.js` and `cookbook.js` each already have) gained the identical Fiber/Net Carbs chip pair and now passes `nutr:{fiber}` into its own `addEntry()` call, which used to omit `nutr` entirely. **Deliberately NOT built: a day-level fiber/net-carbs total.** Zero of the 318 built-in recipes author `fiber_g` (this app has no curated fiber data for them), so a "today's net carbs" figure would be silently wrong — or would need a much heavier "how many logged items are missing data" caveat — for virtually every real day of use right now. Building that now would manufacture the exact silently-wrong trap the audit warned about, not fix it; declined for the same reason gap #03 declined guessing at a DENSITY-routed pantry comparison — an honest per-item figure that's sometimes absent beats a day total that's sometimes lying. Exposed as `window.MCTracker`. |
| `mc-supabase.js` / `mc-sync.js` / `mc-account.js` | Optional login + cross-device sync, ported from 4-Weeks-to-Open- (same Supabase project — one account works in both apps). `mc-supabase.js` is the client + auth (invite-only, no public sign-up) and also exposes `MC_SB.callFunction(name, body)` — a thin `client.functions.invoke()` wrapper used by `cookbook-home.js`'s recipe-import flow to call `fetch-recipe-source`, which resolves with the function's parsed JSON body on **either** success or its own `{ok:false, error}` failure shape (supabase-js's `invoke()` only populates `data` on a 2xx; a non-2xx surfaces as a `FunctionsHttpError` whose real response body lives on `error.context`, which this reads instead of just throwing past the caller) — it only rejects when there's genuinely no structured response at all; `mc-sync.js` mirrors a whitelist of localStorage stores to Supabase's `user_sync` table per signed-in user — `mc_macros_v1` plus every `mc-cookbook:mealplan*` key, `mc-cookbook:userrecipes`, `mc-cookbook:cooked` (**re-audit gap #04 closed a real leak here** — this file's own header long claimed cook-log photos were "device-local by design," which was untrue of the code as shipped: `push()` sends whatever `JSON.parse(localStorage.getItem(key))` returns, unmodified, and `:cooked[id][].photo` held a full base64 JPEG, so every cook-log photo was pushed to Supabase on every sync for every signed-in cook who ever attached one. `mc-photos.js` moving those bytes into IndexedDB and leaving only a boolean `true` marker in `:cooked` fixed it with **no change to this file at all** — the comment became a true description of the code again because the bytes are simply no longer in the JSON `push()` reads), and (**audit C-02**) `mc-cookbook:favorites` + `mc-cookbook:pantry` (each with its own merge strategy — see the file header). Favorites and pantry were absent from the whitelist until C-02: three screens write favorites and Home advertises "☁️ Backed up," but they never left the device. Both are `Array.from(Set)` id lists, so they use the existing `stringSet` union strategy unchanged. **Union alone loses removals, and the re-audit measured it (critical gap #01):** un-checking a grocery row (local `["eggs"]`, remote `["milk","eggs"]`) merged back to `["eggs","milk"]`, and the same held for un-favouriting a recipe and removing a pantry staple — each device kept re-adding what the other had deleted, permanently, since union has no way to say "gone." `mc-setlog.js` (see its own row above) fixes it without touching those three stores' shape: a companion `mc-cookbook:setlog` map of per-item `{on,ts}` states, synced with the **existing `mapByTs`** strategy, and `reconcileSetStores()` subtracts it from the three unions at the end of every `pull()` — deliberately after the whole pull loop (it reads two freshly-merged stores at once, so doing it inside `pullKey` would depend on key order) and before `push()`, so the reconciled array is what this device uploads and the removal travels outward instead of only being applied locally. A store it leaves byte-identical is not rewritten at all, so an untouched store never looks dirty to `push()`; a reconcile write that fails on quota marks the key `blocked` for the same reason `pullKey` does, since pushing the un-reconciled union would undo the very removal being propagated. `mc-cookbook:timecheck` (2026-08-02) closes the last C-02 gap — `{ scopeKey, days, ts }` syncs via a new `replaceByTs` strategy (whole-object last-write-wins on `ts`, the only honest merge for a per-day scalar map with nothing to union). **`mc-cookbook:mealplan` converges on in-place edits now (end-to-end audit).** It was merged with `mergeArrayByField`'s "first occurrence of a uid wins" rule — correct for an append-only store, wrong for this one: `completed`/`completedAt`, `serving` and (via `swapMeal`) `id` are all edited after a meal is created, so local always beat remote and the two devices never agreed. Marking Tuesday's dinner cooked on the phone left the laptop showing it uncooked *permanently*, each device preferring its own copy on every pull. `mergeMealsByUid()` replaces it: `cookbook-home.js`'s `stampMeal()` writes an `mts` (mutation timestamp) on create and on every edit, and the newer stamp wins — the same per-entry tiebreak `mergeMacros()` already applies to logged food entries. Backward compatible by construction: two pre-`mts` meals fall back to local-first byte-for-byte, and a stamped entry beats an unstamped one because only one side knows when it last changed. (The separate `mc-cookbook:cooked` log always did converge — `mergeCookedByRecipe` unions by recipe — so this was the plan's own completion flag drifting, not the cook log.) `mc-cookbook:pantryqty` (roadmap: "Real Pantry Inventory") syncs via a new `mapByTs` strategy — like `replaceByTs` but per-KEY rather than whole-object, since this store is a flat map of independent per-staple entries (`{[pantryKey]: {qty,unit,ts}}`), not a single scalar object: two devices editing two *different* staples' quantities at once must resolve both edits, not have one clobber the other the way a whole-object strategy would. Shares `stringSet`'s known limitation, not a new one — a deletion (clearing a quantity back to unquantified) doesn't propagate as a removal, only additions/updates merge. **Deliberately still not synced**, with reasons in the file: `:photos` (image data doesn't belong in a jsonb row), `:timers` (a running timer is device-local by nature — the phone on the counter is cooking, the laptop is not — and no merge strategy works: union would resurrect cancelled timers on the other device, replace would let a backgrounded device wipe the one you're actually cooking on), and the device/session-local preferences (`:tourSeen`, `:owner`, `:cookfont`, `:lastScreen`, `:lastBackupAt`, the dismissal stamps, and per-recipe check-offs). **Bridge (roadmap B0):** `mc-sync.js` also has a `CONSUME` map that **pulls, read-only,** the workout app's `mc_activity` + `mc_workout_log_v1` from the same `user_sync` table (owner authoritative, `replace` merge, never pushed back — one writer per store); `mc-bridge.js` reads those for a cross-app view. `mc-account.js` is the sign-in-sheet UI, mounted into the Home top bar by `cookbook-home.js` via `window.MCAccount.mount(container)`, and also hosts the Export data / Import data buttons (works whether signed in or not). All three sync modules are no-ops when signed out — nothing changes for a cook who never logs in. **Roadmap B5:** the merge functions (`mergeMacros`, `mergePlan`, `mergeStringSet`, `mergeHistoryBySavedAt`, `mergeArrayByField`, `mergeCookedByRecipe`) are private closures in a browser-only IIFE, so a `module.exports` hook was added as the literal first statement inside it (before the `window.__mcSync`/`MC_SB` guards) — the merge functions are `function` declarations further down the same closure so they're already hoisted and defined regardless of how the guards resolve. `tools/test-mc-sync-merge.js` sandboxes the real file with `vm` (fake `window`/`MC_SB` so the guards return early right after `module.exports` is set) and asserts real conflicting-fixture behavior — now a blocking CI step, not just a local check. `tracker-foodapi.js`/`tracker-calc.js`/`tracker-barcode.js` are generated copies of the workout app's `mc-foodapi.js`/`mc-macrocalc.js`/`mc-barcode.js` via `tools/sync-shared-modules.py` in that repo (which also enforces the byte-identical copies of `mc-bridge.js`/`mc-install.js`/`mc-backup-status.js`/`tools/test-mc-bridge.js`; a CI drift gate in both repos' `pages.yml` fails on any stale copy) — don't hand-edit them here. **Web Locks** (runtime-invisibles audit, delight opportunity 3): `start()`'s initial pull→push cycle and the periodic `flush()` are wrapped in `navigator.locks.request('mc-cookbook-sync', { ifAvailable: true }, …)` — with two tabs open on the same device, each used to run its own pull on load and its own periodic push, interleaving merges and racing to upsert the same `store_key`. A tab that loses the race just skips that one cycle (`ifAvailable` — never queues) since the periodic timer covers it 30s later and the tab actually holding the lock already covers the same data. Falls straight through to running unlocked wherever the Locks API doesn't exist; the exposed `MC_SYNC.pull()`/`MC_SYNC.push()` methods stay unlocked passthroughs for deliberate manual invocation. |
...
| `diagnostics.html` | **Device Check** — the real-device half of the audit's verification, which CI can't do. Standalone and unlinked from nav (same precedent as the Quick Tour). Loads the same script set `index.html` does, then self-tests: install/display mode, service-worker registration + control, precache completeness and app-shell-from-cache (offline readiness), **Cache Storage hygiene** (runtime-invisibles audit — flags any cached entry whose origin isn't this app's own, or whose stored response status isn't 200; `sw.js`'s fetch handler refuses to cache either going forward, but this catches a lingering pre-fix entry or a future regression directly on the device), every `MC*` shared module, localStorage writability + storage quota, Supabase config/sign-in/last-push-pull, wake lock / BarcodeDetector / camera, safe-area insets — and the **real** `window.__mcBoot` boot number that closes audit C-06. **Re-audit gap #04** replaced the old "recipe cover photos: N of 24 cap" check (a claim that stopped being true the moment covers moved off localStorage) with two: whether any legacy base64 is still sitting in `mc-cookbook:photos`/`mc-cookbook:cooked[].photo` waiting to migrate, and a live count read directly from the `mc-cookbook-photos` IndexedDB database — no item cap to report, since that ceiling was only ever localStorage's own quota worked backward into a number. "Copy report" yields a plain-text summary. Open it on each device and mode to fill the PWA matrix. |
...
| `tools/smoke-test.js` | **Blocking CI gate as of 2026-08-02** (previously local-only). Drives the real app in Playwright/Chromium: shell boot, detail hydration, ingredient search, `?cook=1`, the two regressions worth pinning — a timer surviving a step advance and surviving a full page navigation — and, as of the VOC/VOA Kaizen audit wave 7, a simulated full-quota write on `recipe.html` and `collection.html` to prove the storage-full toast actually surfaces there (see `mc-fav.js`'s row and the wave 7 writeup in `ROADMAP.md`). Playwright is installed ad hoc in the `verify` job (see the CI section below), not via a committed `package.json`, so this repo's real dependency footprint is unchanged. Still worth running locally before pushing anything that touches load order, Cooking Mode, the timers, or a write path — CI will catch a regression either way, but locally is faster to iterate on. **Re-audit critical gap #03** added a scenario that searches the live corpus at test time (never a hardcoded recipe ID) for two real recipes that fragment a shared ingredient across unit families, plans exactly those two, then self-verifies the seeded plan actually reproduces `need: null` against the real `MCGrocery.buildGrocery()` output before asserting anything about the DOM — a future corpus edit that happens to un-fragment this specific pair skips the scenario instead of failing on a stale premise. Caught a real accessibility mistake while being written: the fix's first draft marked the disabled 📏 with `aria-disabled="true"`, which Playwright's own actionability check treats as fully inert — the same thing many screen readers do, the opposite of "still tappable to explain itself." The gate now asserts the control stays a real, enabled button. |
| `tools/test-scaling-format.js` | **The serving-scaling quantity formatter, measured against the real corpus** (end-to-end audit). Extracts `prettyNumber`/`smallAmount`/`scaleQuantity` from **both** `cookbook.js`'s and `user-recipes.js`'s source text (they're IIFE-private, and duplicated on purpose) and sweeps all 318 recipes × servings 1–12 — ~15k scaled lines — for two properties: a nonzero authored quantity never renders as `"0"`, and whatever it renders as stays parseable by `mc-grocery.js`'s `parseQty()` so a scaled amount can still be summed into a shopping list. **A second sweep covers the ranges** (re-audit gap #02): the first one gates on `parseQtyNumber()` returning a number, which is precisely what a range does *not* do — so ranges were the one part of the corpus that sweep structurally could not see, which is how they stayed frozen at 1× for a year. The range sweep asserts each of the 18 authored range lines actually moves at every computed serving count (193 renders) and always renders as `"lo to hi"`. The drift assertion between the two copies is the one that keeps the deliberate duplication honest, and it compares `scaleQuantity`'s **source text** now, not only `prettyNumber`'s output — a one-sided range fix sailed straight through the behavioural check alone. |
...
| `tools/test-executive-summary.js` | **Does the Executive Summary (`quick-tour-overview.html`) tell the truth?** Closes the one item the 2026-09-13 re-audit left open after all five critical gaps shipped — this page had **zero** CI coverage: `check-tour-coverage.js` and `test-quick-tour.js` both explicitly skip it (see their own headers) since it stays hand-authored on purpose, same reasoning as roadmap `F6`'s reversal in the sibling workout app. This file doesn't reverse that decision or add the page to `features.js`'s coverage sweep — it verifies specific, real claims against the app's own source and data instead: the hero's recipe-count and serving-scaling-range chips, the Home module list (including that Tracker is correctly excluded from it — the exact shape of two of the five errors `test-quick-tour.js` found), the bottom tab bar's order, the "Eleven dish types" list against `CATEGORY_ORDER`, every collection card's Live/Coming-soon status and Sub-tabs claim against the real `COLLECTIONS` entry (deliberately **not** requiring every live collection to appear — the cards are a curated highlight reel, and the page never claims to be exhaustive), the serving-ladder honesty claim, the retired `Smart Week`/`Time Check` names, the tracker search button's label verbatim, and that the footer CTA links resolve to real files. Proven to fail on eight planted regressions before landing. |
...
| `.github/workflows/pages.yml` | CI, two jobs: **`verify`** (26 blocking gates — syntax, recipe data, doc-drift check, Quick Tour feature coverage, Quick Tour content truth, Executive Summary content truth, bridge + sync-merge tests, set-store removal propagation, kitchen-timer store, split data layer, generated-data freshness, ingredient units/aisle model, search ranking, recipe-capture parser, pantry-quantity engine, serving-scaling quantity formatter, edge-function SSRF guards, SW strategy, backup format, silent-write-path lint, PWA manifest/icon correctness, UI smoke test, kitchen-ergonomics touch-target ratchet, precache freshness, shared-module drift) runs on **pull requests and `main`**; **`deploy`** (`needs: verify`, `main` only) regenerates the SW and publishes to GitHub Pages. See CI / deploy below. |
...
- `macro_profiles` — **per single serving**, identical across every tier a
  ...
  never scale with serving count. Required: `calories`/`protein_g`/`fat_g`/
  `carbs_g`. `fiber_g` is **optional** (re-audit critical gap #05) — none of
  the 318 recipes below author it (no curated fiber data exists for them),
  and it is never backfilled; when a future recipe does author it, it must
  be present and equal across every tier, same as the four required fields
  (`tools/validate-recipes.js` enforces both).
...
- `mc-cookbook:favorites` — JSON array of favorited `recipe_id`s. Hearts on
  ...
  C-02, which found it had never been in the whitelist. **Removals propagate
  via `mc-cookbook:setlog`** (below) as of the re-audit — union alone could not
  carry an un-favourite, so it came back on the next pull.
- `mc-cookbook:pantry` — JSON array of lowercased staple names; suppresses
  ...
  Also synced as of C-02; removals via `:setlog` as of the re-audit.
- `mc-cookbook:setlog` — `{ "<kind>:<id>": { on: 0|1, ts } }`, the per-item
  add/remove state for the three **set-shaped** stores (`fav` → `:favorites`,
  `pan` → `:pantry`, `gro` → `:mealplan:grocery`). A sparse companion to all
  three, not a change to any of their shapes — see `mc-setlog.js`'s row above
  for why `on:1` is load-bearing and why it is written by diffing at `save()`.
  Synced via `mapByTs` (per-key last-write-wins, reused unchanged);
  `mc-sync.js`'s `reconcileSetStores()` subtracts it from the three unions
  after every pull, before `push()`, so the removal travels outward instead of
  only being applied locally. Entries past 90 days are pruned.
- `mc-cookbook:pantryqty` — `{ [pantryKey]: {qty, unit, ts} }`, an optional
  quantity per staple (roadmap: "Real Pantry Inventory"). A separate, sparse
  companion to `:pantry` above, not a change to that store's shape — a
  staple with no entry here is unaffected in every existing code path.
  Synced via `mapByTs` (per-key last-write-wins). Wired into the This Week
  planner's grocery list — see `mc-pantry.js`'s row for the comparison
  engine it feeds and `cookbook-home.js`'s row for the UI.
- `mc-cookbook:mealplan` — the This Week planner `{ meals: [...] }`. Each meal is
  ...
  serving and constant across tiers, so the snapshot never goes stale. Also
  `mts` — a mutation timestamp written by `stampMeal()` on creation and on
  every in-place edit (completion toggle, `updateMeal`, `swapMeal`), so
  `mc-sync.js`'s `mergeMealsByUid()` can tell which device's copy of a meal is
  the newer one. Absent on meals written before it existed, which merge exactly
  as they always did — see the `mc-sync.js` row above.
- `mc-cookbook:mealplan:grocery` — checked grocery merge-keys for the plan.
  Un-checking propagates across devices via `:setlog` as of the re-audit.
...
- `mc-cookbook:cooked` — `{ [recipe_id]: [{ at, photo }] }` cook log, written by
  ...
  planned-vs-cooked stat. Synced via `mc-sync.js` when signed in. **`photo` is
  a boolean now, not the photo (re-audit gap #04):** it used to be a full
  base64 JPEG string, which meant every cook-log photo's bytes were pushed to
  Supabase on every sync despite `mc-sync.js`'s own comment claiming they
  were device-local — untrue of the code as shipped. `photo: true` is now
  just an "a photo exists in `mc-photos.js`'s IndexedDB library for this
  entry" marker; every existing truthy check on `e.photo` (`photoFor()`'s
  cook-log fallback, the cook-log gallery) keeps working unchanged on `true`
  exactly as it did on a real string. A legacy entry with the raw base64
  still in it migrates to this shape the first time `mc-photos.js` boots.
...
- `mc-cookbook:photos` — **re-audit gap #04 emptied this out.** Was
  `{ [recipe_id]: dataURL }`, one cover photo per recipe, capped at
  `MAX_RECIPE_PHOTOS` (24, oldest evicted, added by C-12) purely because
  localStorage's shared origin quota couldn't hold more. The photo bytes
  themselves now live in IndexedDB (`mc-photos.js`, database
  `mc-cookbook-photos`) with no item cap; this key stays only as the landing
  spot a legacy backup's raw base64 briefly reappears in before
  `runMigration()` moves it and strips the string back out — on a normally-
  migrated device it's `{}`. **Not synced** (image data doesn't belong in a
  jsonb row — unchanged); no longer in the backup file at all going forward,
  since there's nothing left in it to export (a restored OLD backup still
  gets every photo it was made with — see `mc-export.js`'s row).
- `mc-cookbook:photosMigratedV1` — `"1"` once `mc-photos.js`'s one-time
  legacy-photo migration has run on this device (re-audit gap #04). Gates
  the ordinary boot-time migration only; `mc-export.js`'s restore path calls
  `MCPhotos.runMigration()` unconditionally regardless of this flag, since a
  restored old backup can reintroduce legacy data after this flag is already
  set.
...
- **`verify`** — runs on **pull requests and on `main`**. Twenty-five gates, all
  blocking:
  ...
  4. `tools/check-tour-coverage.js --check` — the cookbook's first mechanized
     ...
     the gate fails if any entry has zero mention in the tour's real text —
     **`quick-tour.html` + `quick-tour-data.js`**, since the slides moved into
     the latter when `quick-tour-full.html` started rendering them too. The
     rule is about the tour's prose, not the file holding it; reading only the
     page would have failed on every feature the moment the text moved. Deliberately checks the *tour* only, not
     ...
     a planted missing-feature entry before landing. That page's own factual
     accuracy — not coverage, truth — is gate 16b (`test-executive-summary.js`)
     below, added as a re-audit follow-up once the deliberate coverage gap
     here turned out to mean *no* gate read the page at all.
  5. `tools/test-mc-bridge.js` + `tools/test-mc-sync-merge.js` — cross-app read
     layer and sync-merge logic (roadmap B5, audit C-02).
  5b. `tools/test-mc-setlog.js` — **removals actually propagate** (re-audit
     critical gap #01). `stringSet` is a pure union, so the three set-shaped
     stores carried additions across devices and silently discarded
     REMOVALS: un-checking a grocery row, un-favouriting a recipe and
     removing a pantry staple all came back on the next pull, permanently,
     because each device preferred its own copy every time. This gate pins
     `mc-setlog.js`'s per-item `{on,ts}` register **end to end through
     `mc-sync.js`'s real `reconcileSetStores()`** against a fake
     `localStorage`, not just its pure helpers — including that a second
     reconcile is a no-op (a store needlessly rewritten would be re-uploaded
     every cycle) and that a missing or corrupt log is inert rather than
     destructive. Proven to fail on four planted regressions before landing:
     a bare-tombstone log with no `on:1` record (breaks remove-then-re-add —
     3 assertions), `applyRemovals()` made inert (5), reconciliation moved
     after `push()` (1), and a reconcile that rewrites an unchanged store (2).
  ...
  11. `tools/test-mc-import.js` — the recipe capture parser (roadmap Phase C).
      Shipped with PR #146 but never actually wired into this workflow until
      now — a real gap this same change closed alongside adding gate 12
      below, caught while adding a third new test file to the same area.
      **Re-audit critical gap #05** added two cases pinning `fiberContent`
      extraction: present (extracted into `macros.fiber_g` alongside the
      other four fields) and absent (honestly `null`, not `0` or dropped
      from the object) — proven to fail on a planted regression (the
      extraction line stubbed to always return `null`) before landing.
  12. `tools/test-mc-pantry.js` + `tools/test-mc-grocery.js` — the pantry-
      quantity comparison engine and the per-row `need` field that feeds it
      (roadmap: "Real Pantry Inventory," first UI slice). `test-mc-pantry.js`
      shipped with PR #149, also never wired in until now.
  13. `tools/test-mc-scale.js` + `tools/test-mc-timeline.js` — the flagship
      roadmap's remaining two engines: bi-directional macro/ingredient
      scaling (`mc-scale.js`'s `solveScaleForTarget()`, §2.4) and the
      multi-dish cook timeline's critical-path math (`mc-timeline.js`'s
      `computeTimeline()`, §2.3). Both pure functions, both pinned against
      hand-computed expected results the same way `test-mc-pantry.js` pins
      `mc-pantry.js` above.
  14. `tools/test-scaling-format.js` — the serving-scaling quantity formatter,
      swept over the real corpus (318 recipes × servings 1–12, ~15k scaled
      ingredient lines). Pins the end-to-end audit finding that
      `prettyNumber()`'s `if (frac < 0.06) return String(whole)` rendered a
      **nonzero** scaled amount as `"0"` whenever there was no whole part —
      25 ingredient lines across 12 recipes, all of them batch-yield desserts
      or single-tier recipes being scaled *down* ("1/2 tsp Vanilla bean paste"
      at 1 serving of a 24-serving cheesecake became "0 tsp"), rendered in the
      mise-en-place list and **read aloud verbatim** by Cooking Mode's
      `speakIngredients()`. Also asserts the second property the fix depends
      on — a scaled amount stays parseable by `mc-grocery.js`'s own
      `parseQty()`, which is why the replacement is a small decimal and not
      "a pinch" — and that `cookbook.js`'s and `user-recipes.js`'s
      ...
      **Extended by re-audit gap #02** to cover ranged amounts, which the
      original corpus sweep structurally could not see (it gates on
      `parseQtyNumber` returning a number — a range returns null, which is
      exactly why `"2-3 cloves garlic"` stayed `"2-3"` at every serving count
      for a year while every other line on the page scaled). Adds a second
      sweep over the corpus's 18 authored range lines (193 renders), the
      `"8-inch"`/`"4-inch"` guard that keeps mis-keyed pan dimensions out of
      the range path, and a real **source-text** drift assertion on
      `scaleQuantity` — the pre-existing drift check only compared
      `prettyNumber`, so a one-sided range fix passed it. Proven against four
      planted regressions: the pre-fix pass-through (8 fail), a one-sided fix
      with `user-recipes.js` left alone (2), the separator regressed to a bare
      hyphen (6), and the both-sides-numeric guard weakened so `"8-inch"`
      scales (2).
  15. `tools/test-mc-photos.js` — the photo library's pure id/legacy-value
      helpers (re-audit critical gap #04: base64 recipe-cover and cook-log
      photos in localStorage, capped at 24/12 items purely because that
      store is the wrong primitive for binary blobs, moved into a new
      IndexedDB-backed `mc-photos.js` — see that file's row above). Pins
      `coverId()`/`cookId()`'s id scheme and `isLegacyPhotoValue()`'s
      length-floor detection (proven against a planted floor-value
      regression before landing). The IndexedDB-dependent half — migration,
      the warm-cache read path, the write API — can't run under Node and is
      proven by `tools/smoke-test.js` instead.
  16. `tools/test-quick-tour.js` — **does the Quick Tour tell the truth?**
  ...
  16b. `tools/test-executive-summary.js` — **does the Executive Summary tell
      the truth?** Gate 16 covers the tour; this covers `quick-tour-overview.html`,
      which had **zero** CI coverage until now — the re-audit's own last open
      item. Deliberately does not reverse the decision to keep that page
      hand-authored (see `features.js`'s header again): it checks real,
      specific claims — the recipe-count and serving-scaling chips, the Home
      module list (and that Tracker is correctly excluded from it — the same
      error shape gate 16 found twice), the bottom tab bar, the dish-type
      list against `CATEGORY_ORDER`, every shown collection card's status
      against the real `COLLECTIONS` entry (not requiring the page to list
      every live collection — it never claims to), the serving-ladder claim,
      retired feature names, the tracker search button's label, and the
      footer CTA links — against the app's own source and data. Proven to
      fail on eight planted regressions before landing.
  17. `tools/test-fetch-recipe-source.js` — **the first CI coverage of any
      ...
      function's own header said so, neither of which is a test. 73 assertions
      over the real private-range tables (both IPv4 and IPv6, including
      `::ffff:127.0.0.1` — loopback wearing a v6 costume), `validateUrl`'s
      scheme/port/credential rejections, `hostIsSafe`'s fail-closed paths
      (unresolvable host, and a host answering with one public *and* one
      private record — the DNS-rebinding shape), the CORS allow-list's
      lookalike-domain rejections, and `safeFetchHtml`'s manual redirect loop.
      That last one is the point: **a public URL that 302s onto
      `169.254.169.254` must be blocked, and the metadata address must never
      be fetched at all.** Runs under Node, not Deno — so it lifts the guards
      out of the file's own text and strips the TypeScript annotations by a
      list of exact, *counted* replacements, meaning a source edit that moves
      an annotation fails this gate loudly instead of silently testing a stale
      copy. It deliberately does not ask the function to `export` anything:
      that would make the committed file differ from the deployed one. Proven
      to fail on two planted regressions before landing — dropping the
  ...
  18. `tools/test-sw-strategy.js` — service-worker stale-while-revalidate (LS-4).
  19. `tools/test-mc-export.js` — backup format round trip + legacy files (C-01).
  20. `tools/check-write-paths.js` — no `localStorage.setItem()` call is left
  ...
  21. `tools/test-manifest.js` — PWA manifest + icon correctness, pure file and
  ...
  22. `tools/smoke-test.js` — the one gate that opens a real page in a real
      ...
      **Re-audit critical gap #03** added a self-verifying pantry-disclosure
      scenario — see this file's own row above for what it covers and the
      accessibility mistake it caught while being written. **Re-audit
      critical gap #04** extended the existing photo-resolution-chain
      scenario to prove the base64→IndexedDB migration end to end: a fresh,
      isolated browser context (needed because the shared `page`'s
      migration flag would already be set from earlier scenarios' own
      visits — seeding the legacy photo via `addInitScript` guarded on that
      same flag, so it stops reseeding after the first real migration)
      confirms the legacy string is actually stripped from localStorage and
      the record actually lands in IndexedDB, that a brand-new cover write
      never touches localStorage at all, that a cook-log photo attach/
      remove round-trips and cleans up its objectURL, and that a backup
      export no longer contains base64 image data. Caught a real regression
      while being written: `renderPhotoWidget()`'s "add a photo" eyebrow
      button could be decided before `mc-photos.js`'s IndexedDB cache
      warmed, and nothing repainted just that widget once it did —
      `refreshPhotoWidget()` (`cookbook.js`) is the fix, same narrow-scope
      reasoning as `renderHero()` itself. **Re-audit critical gap #05**
      added a fiber/net-carbs scenario: since nothing in the real 318-recipe
      corpus authors `fiber_g`, the only way to exercise the whole chain for
      real is to hand-type a recipe through `mc-recipe-form.js`'s new Fiber
      field and follow that number through the recipe page's macro card,
      the "Log to tracker" sheet, and the tracker's own Nutrition Facts
      sheet for the entry logging creates — 18 assertions, proven to fail
      (isolated to exactly the macro-card checks) against a planted
      regression before landing.
  23. `tools/check-a11y.mjs` — the cookbook's first accessibility gate (VOC/VOA
      ...
      **Ratcheted 84 → 62** by the end-to-end audit's A-05 pass, and its new
      ...
      by the exemption.
  24. `tools/build-sw.py --check` — precache list is current.
  25. Shared-module drift vs the 4-Weeks-to-Open- canonical copies (LS-1).
...
node tools/test-mc-photos.js
node tools/test-quick-tour.js
node tools/test-executive-summary.js
