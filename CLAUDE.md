...
| `mc-import.js` | **Recipe capture parsing — `FLAGSHIP_COOKBOOK_ROADMAP.md`'s "Recipe Capture & Import Pipeline," now wired end to end for the URL path.** Pure, DOM-free (regex over raw HTML text, not a `DOMParser`, so it behaves identically in a browser and under Node in CI): given fetched page HTML, returns a best-effort recipe shaped like `mc-recipe-form.js`'s own save payload, including a `macros` field that form now actually saves (see that row above — it used to only show it). Two paths — schema.org/Recipe JSON-LD (reliable, structured: `recipeIngredient`, `recipeInstructions` including nested `HowToSection`, `nutrition`, ISO 8601 `prepTime`/`cookTime`) tried first, then a heuristic "Ingredients"/"Instructions" heading-and-`<li>` scan that returns `null` rather than a half-built guess when it can't find both sections. Ingredient-line splitting (`parseIngredientLine`) and per-ingredient `category` (`guessCategory`, keyword-based) are both explicitly best-effort. Called by `cookbook-home.js`'s `openImportFromUrl()` after `fetch-recipe-source` returns HTML; **no OCR/photo-capture path exists** — that third capture method from the roadmap is deliberately unbuilt, same "ship the narrowest version" discipline as everything else in this pipeline so far. Pinned by `tools/test-mc-import.js` against JSON-LD and heuristic fixtures, a malformed-JSON-LD-must-not-crash case, and a neither-section-found-must-return-null case. Loaded in `index.html` right before `mc-recipe-form.js`. Exposed as `window.MCImport`. |
...
- **`verify`** — runs on **pull requests and on `main`**. Twenty-five gates, all
  ...
  22. `tools/smoke-test.js` — the one gate that opens a real page in a real
      ...
      **Re-audit critical gap #03** added a self-verifying pantry-disclosure
      ...
      reasoning as `renderHero()` itself.
