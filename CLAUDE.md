...
| `.github/workflows/pages.yml` | CI, two jobs: **`verify`** (9 blocking gates — syntax, recipe data, doc-drift check, bridge + sync-merge tests, kitchen-timer store, SW strategy, backup format, precache freshness, shared-module drift) runs on **pull requests and `main`**; **`deploy`** (`needs: verify`, `main` only) regenerates the SW and publishes to GitHub Pages. See CI / deploy below. |
...
- **`verify`** — runs on **pull requests and on `main`**. Nine gates, all
  blocking:
  ...
  6. `tools/test-sw-strategy.js` — service-worker stale-while-revalidate (LS-4).
  7. `tools/test-mc-export.js` — backup format round trip + legacy files (C-01).
  8. `tools/build-sw.py --check` — precache list is current.
  9. Shared-module drift vs the 4-Weeks-to-Open- canonical copies (LS-1).
