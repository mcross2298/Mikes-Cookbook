...
| `.github/workflows/pages.yml` | CI, two jobs: **`verify`** (20 blocking gates — syntax, recipe data, doc-drift check, Quick Tour feature coverage, bridge + sync-merge tests, kitchen-timer store, split data layer, generated-data freshness, ingredient units/aisle model, search ranking, recipe-capture parser, pantry-quantity engine, SW strategy, backup format, silent-write-path lint, PWA manifest/icon correctness, UI smoke test, kitchen-ergonomics touch-target ratchet, precache freshness, shared-module drift) runs on **pull requests and `main`**; **`deploy`** (`needs: verify`, `main` only) regenerates the SW and publishes to GitHub Pages. See CI / deploy below. |
...
- **`verify`** — runs on **pull requests and on `main`**. Twenty-one gates, all
  blocking:
  ...
  13. `tools/test-mc-scale.js` + `tools/test-mc-timeline.js` — the flagship
      ...
      `mc-pantry.js` above.
  14. `tools/test-sw-strategy.js` — service-worker stale-while-revalidate (LS-4).
  15. `tools/test-mc-export.js` — backup format round trip + legacy files (C-01).
  16. `tools/check-write-paths.js` — no `localStorage.setItem()` call is left
  ...
  17. `tools/test-manifest.js` — PWA manifest + icon correctness, pure file and
  ...
  18. `tools/smoke-test.js` — the one gate that opens a real page in a real
  ...
  19. `tools/check-a11y.mjs` — the cookbook's first accessibility gate (VOC/VOA
  ...
  20. `tools/build-sw.py --check` — precache list is current.
  21. Shared-module drift vs the 4-Weeks-to-Open- canonical copies (LS-1).
