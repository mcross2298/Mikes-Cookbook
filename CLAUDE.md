...
| `mc-import.js` | **Recipe capture parsing — first slice of `FLAGSHIP_COOKBOOK_ROADMAP.md`'s "Recipe Capture & Import Pipeline."** Pure, DOM-free (regex over raw HTML text, not a `DOMParser`, so it behaves identically in a browser and under Node in CI): given fetched page HTML, returns a best-effort recipe shaped like `mc-recipe-form.js`'s own save payload, plus a `macros` field that form doesn't accept yet. Two paths — schema.org/Recipe JSON-LD (reliable, structured: `recipeIngredient`, `recipeInstructions` including nested `HowToSection`, `nutrition`, ISO 8601 `prepTime`/`cookTime`) tried first, then a heuristic "Ingredients"/"Instructions" heading-and-`<li>` scan that returns `null` rather than a half-built guess when it can't find both sections. Ingredient-line splitting (`parseIngredientLine`) and per-ingredient `category` (`guessCategory`, keyword-based) are both explicitly best-effort. **Not yet wired to anything** — no UI and no OCR photo-capture path yet; this module only does the parsing half. The `fetch-recipe-source` edge function that gets a pasted URL's HTML past CORS now exists (see that row below) but nothing calls it yet — the two pieces aren't connected. Pinned by `tools/test-mc-import.js` against JSON-LD and heuristic fixtures, a malformed-JSON-LD-must-not-crash case, and a neither-section-found-must-return-null case. Exposed as `window.MCImport`. |
| `supabase/functions/fetch-recipe-source/index.ts` | **The server-side half of the capture pipeline's URL path.** A narrow Deno edge function on the shared Supabase project (`dhlxmoyjfxohbeiexwnr` — the same one `mc-supabase.js` points at, see the `mc-supabase.js` row below): given `{url}`, fetches it server-side and returns `{ok, html, finalUrl}`, since a static site can't fetch an arbitrary third-party URL client-side past CORS. Does no parsing — that stays `mc-import.js`'s job. **Hardened against SSRF**, since it fetches whatever URL a caller sends: every hostname (the initial URL *and* each redirect hop, followed manually via `redirect: "manual"`, never auto-followed) is DNS-resolved and checked against private/loopback/link-local/reserved ranges — including `169.254.169.254`, the common cloud-metadata address — before being fetched; only http/https, only default ports, no embedded credentials; an 8s timeout and a 3MB response cap (truncates rather than hangs). Deployed with **`verify_jwt: true`** (signed-in cooks only) — the project already has public utility functions (`food`, `parse-food`), but the deploy tool's own guidance defaults a *new* function to JWT-required unless it implements its own auth, and a URL-fetch proxy is a meaningfully larger abuse surface than a food lookup; this is a deliberate, flagged trade-off (recipe import via URL needs sign-in even though hand-typing one doesn't), not an oversight. CORS is allow-listed to `https://mcross2298.github.io` plus localhost for local dev — worth being honest that CORS is a browser-enforced boundary, not a defense against a non-browser caller; the SSRF/size/timeout hardening plus the JWT gate are what actually bound abuse. **Deployed and `ACTIVE`**, but live HTTP behavior (the SSRF blocks, the JWT gate, real-page fetches) could not be exercised from the session that built it — this repo's outbound network policy blocks reaching `*.supabase.co` directly, so verification was code review only, not a live request/response transcript. Not precached by `tools/build-sw.py` (server code, not a client asset — that script's top-level scan is deliberately non-recursive and `.ts` isn't a precached extension anyway, so nothing needed regenerating for this file). No CI gate covers Deno/edge-function code today — none of the 18 `verify` gates run outside Node. |
