...
- **`verify`** — runs on **pull requests and on `main`**. Twenty gates, all
  blocking:
  ...
  13. `tools/test-sw-strategy.js` — service-worker stale-while-revalidate (LS-4).
  14. `tools/test-mc-export.js` — backup format round trip + legacy files (C-01).
  15. `tools/check-write-paths.js` — no `localStorage.setItem()` call is left
  ...
  16. `tools/test-manifest.js` — PWA manifest + icon correctness, pure file and
  ...
  17. `tools/smoke-test.js` — the one gate that opens a real page in a real
  ...
  18. `tools/check-a11y.mjs` — the cookbook's first accessibility gate (VOC/VOA
  ...
  19. `tools/build-sw.py --check` — precache list is current.
  20. Shared-module drift vs the 4-Weeks-to-Open- canonical copies (LS-1).
