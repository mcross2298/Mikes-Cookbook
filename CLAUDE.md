...
- `mc-cookbook:pantryqty` — `{ [pantryKey]: {qty, unit, ts} }`, an optional
  ...
  Synced via `mapByTs` (per-key last-write-wins). Not yet wired to any UI —
  see `mc-pantry.js`'s row for the comparison engine it feeds.
